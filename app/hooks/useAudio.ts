...
export function useAudio() {
  ...
  const tone = useCallback(
    (
      ...
      try {
        ...
        /* ignore if ctx closed */
      }
    },
    [getCtx],
  );

  // Keep the scheduler ref fresh after every render so setTimeout always calls the latest closure
  useEffect(() => {
  ...
  const playCorrect = useCallback(() => {
    ...
    tone(C5, 0.1, 'sine', 0.3, t);
    tone(E5, 0.1, 'sine', 0.3, t + 0.1);
    tone(G5, 0.18, 'sine', 0.3, t + 0.2);
    tone(C6, 0.3, 'sine', 0.28, t + 0.35);
  }, [getCtx, tone]);
  ...
  useEffect(() => {
    const bgState = bgStateRef.current;
    return () => {
      bgState.playing = false;
      if (bgState.timer) clearTimeout(bgState.timer);
