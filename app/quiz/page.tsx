...
const EMOJIS_WRONG = ["😅", "💪", "🌈", "🔄", "😊"];
const OPTION_COLORS = [
  { bg: "bg-yellow-300 hover:bg-yellow-400 border-yellow-500", selected: "bg-yellow-500 border-yellow-700 scale-105", text: "text-yellow-900" },
  { bg: "bg-sky-300 hover:bg-sky-400 border-sky-500", selected: "bg-sky-500 border-sky-700 scale-105", text: "text-sky-900" },
  { bg: "bg-pink-300 hover:bg-pink-400 border-pink-500", selected: "bg-pink-500 border-pink-700 scale-105", text: "text-pink-900" },
];
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-200 via-yellow-100 to-pink-200 flex items-center justify-center p-4">
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-white rounded-3xl shadow-xl p-6 mb-4 border-4 border-purple-200">
          ...
          <p className="text-center text-xl md:text-2xl font-bold text-purple-800 leading-relaxed">{q.questionText}</p>
        </div>
        ...
        <div className="space-y-3 mb-4">
          {opts.map((opt, i) => {
            ...
            if (!showResult) cls += `${colors.bg} ${colors.text} border-transparent`;
            else if (isCorrect) cls += "bg-green-400 border-green-600 text-green-900 scale-105";
            else if (isSelected) cls += "bg-red-400 border-red-600 text-red-900";
            else cls += "bg-gray-100 border-gray-200 text-gray-400 opacity-60";
            return (
              <button key={opt.value} className={cls} onClick={() => handleAnswer(opt.value)} disabled={!!chosen}>
                <span className="bg-white/50 rounded-full w-10 h-10 flex items-center justify-center font-extrabold text-xl shrink-0">{opt.label}</span>
                <span>{opt.text}</span>
        ...
        {/* Feedback */}
        {feedback && (
          <div className={`rounded-3xl p-4 text-center border-4 mb-4 ${feedback === "correct" ? "bg-green-100 border-green-400 text-green-800" : "bg-orange-100 border-orange-400 text-orange-800"}`}>
            <div className="text-5xl mb-1">{feedbackEmoji}</div>
            <p className="font-extrabold text-xl">{feedback === "correct" ? "Amazing! That's correct! 🌟" : `Oops! The answer is "${q.correctAnswer}" 💪`}</p>
            <button onClick={next} className="mt-3 bg-purple-500 hover:bg-purple-600 text-white font-extrabold text-lg px-8 py-3 rounded-full shadow-lg transition-all hover:scale-105">
              {index + 1 >= shuffled.length ? "See My Score! 🏆" : "Next Question →"}
            </button>
          </div>
        )}
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-200 via-yellow-100 to-pink-200 flex items-center justify-center p-4">
      <div className="w-full max-w-xl text-center">
        ...
        <div className="space-y-4">
          {sets.map((set) => {
            const cfg = setConfig[set] ?? { emoji: "🎯", color: "from-teal-400 to-cyan-500", desc: "Try this quiz!" };
            return (
              <button key={set} onClick={() => onSelect(set)}
                ...
                <div>
                  ...
                  <div className="text-white/80 text-base font-medium">{cfg.desc}</div>
                </div>
              </button>
            );
          })}
        </div>
        <p className="mt-8 text-purple-500 font-medium">30 questions per quiz • Choose wisely! 🦄</p>
      </div>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-200 via-yellow-100 to-pink-200 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full text-center border-4 border-purple-200">
        ...
        <div className="space-y-3">
          <button onClick={onRetry} className="w-full bg-purple-500 hover:bg-purple-600 text-white font-extrabold text-xl py-4 rounded-full shadow-lg transition-all hover:scale-105">
            🔄 Try Again
          </button>
          <button onClick={onRestart} className="w-full bg-sky-400 hover:bg-sky-500 text-white font-extrabold text-xl py-4 rounded-full shadow-lg transition-all hover:scale-105">
            🏠 Choose Another Quiz
          </button>
        </div>
      </div>
